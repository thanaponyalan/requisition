...
function get_credit(){
	$username   = "0849555096";
    $password   = "173056";
  $result = sms::check_credit($username,$password,"credit_remain");// "credit_remain" : Standard / "credit_remain_premium" : premium
  //print_r($result);
  return $result;
}

function sendBulk($username,$password,$msisdn,$message,$sender,$ScheduledDelivery="",$force="standard"){//ฟังก์ชั่นสร้างเอง
...
function send_sms($from,$to,$message,$user_id){
    global $dbPrefig;
//    print "$from,$to[0],$message,$user_id";
//    print_r($to);
    $username   = "0849555096";
    $password   = "173056";
    ...
    for($i=0;$i<count($to);$i++){
     $result = sendBulk($username,$password,$to[$i],$message,$from,"","standard"); //==>>ส่ง SMS 
        $data['destination']="'".$to[$i]."'";
      if(strlen($data['destination'])==12&&is_numeric(substr($data['destination'],1,10))){
        $data['sms_result']="'".json_encode($result)."'";
...
//        print $result[1];
        ...
        //insertTb($dbPrefig."sms_transaction", $data);
      }else{
        return false;
        }
    }
...
function sendWarnSMS($from,$to,$message,$user_id,$userID){
//    print "$from,$to[0],$message,$user_id";
//    print_r($to);
    $username   = "0849555096";
    $password   = "173056";
    ...
    for($i=0;$i<count($to);$i++){
     $result = sendBulk($username,$password,$to[$i],$message,$from,"","standard"); //==>>ส่ง SMS 
        $data['destination']="'".$to[$i]."'";
        $logData=array('via'=>'"SMS"','mobile'=>'"'.$to[$i].'"','msg'=>'"'.$message.'"','user_id'=>'"'.$userID.'"');
        insertTb('msgHist',$logData);
        $remainSMS--;
        updateTb('msgData',array('remainSMS'=>'"'.$remainSMS.'"'),' user_id="'.$userID.'" and isConfirm="0"');
      if(strlen($data['destination'])==12&&is_numeric(substr($data['destination'],1,10))){
        $data['sms_result']="'".json_encode($result)."'";
